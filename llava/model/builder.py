...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map=None, device="cuda", use_flash_attn=False, 
    ...
    if 'llava' in model_name.lower():
        ...
        # ===== Add Attention Actor config attributes before loading =====
        ...
        cfg_pretrained.actor_dropout = actor_dropout
        cfg_pretrained.actor_top_k = actor_top_k
        cfg_pretrained.actor_tau = actor_tau
        # ====================================================
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                cfg_pretrained.actor_top_k = actor_top_k
                cfg_pretrained.actor_tau = actor_tau
                model = LlavaMptForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
                ...
                # Move model to target device (required when low_cpu_mem_usage=True)
                model.to(device)
    else:
        # Load language model
    ...
    if 'llava' in model_name.lower():
        ...
        # Explicitly move vision tower to the specified device
    ...
    # Explicitly move the entire model to the target device (not using device_map)
    if device != "cpu":
