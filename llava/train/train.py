...
def train(attn_implementation=None):
    ...
    #--------------------------------------------------------------------------------
    #导入actor的权重
    actor = freeze_all_but_actor(model)
    actor_ckpt=os.environ.get("ACTOR_CKPT","").strip()
    if actor_ckpt:
        ckpt = torch.load(actor_ckpt, map_location="cpu")
        actor.load_state_dict(ckpt, strict=True)
    else:
        print("[Actor] ACTOR_CKPT not set; train from scratch.")
    #--------------------------------------------------------------------------------
    ...
    if training_args.local_rank == 0 or training_args.local_rank == -1:
        save_actor_only(model, training_args.output_dir)
        # （可选）保存一个简单的 actor 配置，方便复现
        with open(os.path.join(training_args.output_dir, "actor_config.json"), "w") as f:
            json.dump({
