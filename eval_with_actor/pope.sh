...
# 使用方式:
#   bash pope.sh <MODEL_PATH> <ACTOR_CKPT>
# 示例:
#   bash pope.sh /data/users/Actor/CrossAttentionActor/1_results_datacleaned /data/users/Actor/CrossAttentionActor/1_results_datacleaned/actor_visualizations/actor_weights_step_13000.pt
...
NEW_PATH="/data/users/Actor/DynamicVLM/1_results_datacleaned/eval_pope"
...
CUDA_VISIBLE_DEVICES=${gpu_list} python -m llava.eval.model_vqa_loader \
    ...
    --question-file /data/users/Actor/Attention_Actor/playground/data/eval/pope/llava_pope_test.jsonl \
    --image-folder /data/users/Actor/Attention_Actor/playground/data/eval/pope/val2014 \
    --answers-file $NEW_PATH/answers/llava-v1.5-7b.jsonl \
    --temperature 0 \
    --conv-mode vicuna_v1 \
    ${ACTOR_ARGS}

python /data/users/Actor/CrossAttentionActor/llava/eval/eval_pope.py \
    --annotation-dir /data/users/Actor/Attention_Actor/playground/data/eval/pope/coco \
    --question-file /data/users/Actor/Attention_Actor/playground/data/eval/pope/llava_pope_test.jsonl \
