...
# 使用方式:
#   bash vizwiz.sh <MODEL_PATH> <ACTOR_CKPT>
# 示例:
#   bash vizwiz.sh /data/users/Actor/CrossAttentionActor/1_results_datacleaned /data/users/Actor/CrossAttentionActor/1_results_datacleaned/actor_visualizations/actor_weights_step_13000.pt
...
NEW_PATH="/data/users/Actor/DynamicVLM/1_results_datacleaned/eval_vizwiz"
...
CUDA_VISIBLE_DEVICES=${gpu_list} python -m llava.eval.model_vqa_loader \
    ...
    --question-file /data/users/Actor/Attention_Actor/playground/data/eval/vizwiz/llava_test.jsonl \
    --image-folder /data/users/Actor/Attention_Actor/playground/data/eval/vizwiz/test \
...
python /data/users/Actor/CrossAttentionActor/scripts/convert_vizwiz_for_submission.py \
    --annotation-file /data/users/Actor/Attention_Actor/playground/data/eval/vizwiz/llava_test.jsonl \
    --result-file $NEW_PATH/answers/llava-v1.5-7b.jsonl \
    --result-upload-file $NEW_PATH/answers_upload/llava-v1.5-7b.json
