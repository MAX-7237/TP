...
# 使用方式:
#   bash qbench.sh <dev|test> <MODEL_PATH> <ACTOR_CKPT>
# 示例:
#   bash qbench.sh dev /data/users/Actor/CrossAttentionActor/1_results_datacleaned /data/users/Actor/CrossAttentionActor/1_results_datacleaned/actor_visualizations/actor_weights_step_13000.pt
...
NEW_PATH="/data/users/Actor/DynamicVLM/1_results_datacleaned/eval_qbench"
...
CUDA_VISIBLE_DEVICES=${gpu_list} python -m llava.eval.model_vqa_qbench \
    ...
    --questions-file /data/users/Actor/Attention_Actor/playground/data/eval/qbench/llvisionqa_$SPLIT.json \
    --answers-file $NEW_PATH/answers/llvisionqa_$SPLIT_answers.jsonl \
    --conv-mode llava_v1 \
    --lang en \
    ${ACTOR_ARGS}
